...
def parseNmapScan(results):
    ...
    for line in lines:
        ...
        if ('tcp' in line or 'udp' in line) and 'open' in line \
            and not 'filtered' in line and not 'Discovered' in line:
            while '  ' in line:
                line = line.replace('  ', ' ')
    ...
    for service in services:
        ...
        if service in KNOWN_SERVICES:
            try:
                ...
                printDbg('No module available for %s - %s' % (service,
                         port))
        else:
            printDbg('No module available for %s - %s' % (service,
                     port))
...
def conductHeavyNmap(target):
    ...
    try:
        ...
        content = '%s' % printInBox(TCPSCAN, tcpResults)
        path = writeToFile(target, NAME, content)
        printPlus('Finished heavy nmap scan: %s/%s/nmap_heavy.txt'
                  % (os.getcwd(), target))
    except KeyboardInterrupt:
        printMinus('Skipping:\n\t%s' % TCPSCAN)
    except Exception:
    ...
    try:
        ...
        path = writeToFile(target, NAME, content)
        printPlus('Finished UDP scan: %s/%s/nmap_heavy.txt'
                  % (os.getcwd(), target))
    except KeyboardInterrupt:
        printMinus('Skipping:\n\t%s' % UDPSCAN)
    except Exception:
...
def ftp(target, ports):
    ...
    NMAPSCAN = 'nmap -T4 -p %s -sV -sC --script="%s" %s' % (portString,
            SCRIPTS, target)
    try:
...
def smtp(target, ports):
    ...
    for port in ports:
        port = port.split('/')[0]
        portString += '%s,' % port
    NMAPSCAN = 'nmap -T4 -p %s -sV --script="smtp-vuln*" %s' \
        % (portString, target)
    try:
    ...
    SCAN1 = \
        'smtp-user-enum -M EXPN -U /usr/share/fern-wifi-cracker/extras/wordlists/common.txt -t %s' \
        % target
    try:
...
def pop3(target, ports):
    ...
    for port in ports:
        ...
        portString += '%s,' % port
    NMAPSCAN = \
        'nmap -T4 -p %s -sV --script="pop3-capabilities,pop3-ntlm-info" %s' \
        % (portString, target)
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
    ...
    BRUTE = 'nmap -T4 -p %s --script="pop3-brute" %s' % (portString,
            target)
    try:
...
def imap(target, ports):
    ...
    for port in ports:
        ...
        portString += '%s,' % port
    NMAPSCAN = \
        'nmap -T4 -p %s -sV --script="imap-capabilities,imap-ntlm-info" %s' \
        % (portString, target)
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
    ...
    BRUTE = 'nmap -T4 -p %s --script="imap-brute" %s' % (portString,
            target)
    try:
        bruteResults = subprocess.check_output(BRUTE, shell=True)
...
def smb(target, ports):
    ...
    SCAN1 = \
        'nmap -T4 -sV -sC --script="smb-vuln-*,samba-vuln-*" -p 445,139 %s' \
        % target
    SCAN2 = \
        'nmap -T4 -sU -sV -sC --script="smb-vuln-*" -p U:137,T:139 %s' \
        % target
    try:
        scan1Results = subprocess.check_output(SCAN1, shell=True)
        ...
        content = '%s\n%s' % (printInBox(SCAN1, scan1Results),
                              printInBox(SCAN2, scan2Results))
        path = writeToFile(target, NAME, content)
        printPlus('Finished investigating SMB Vulnerabilities: %s'
                  % path)
    except KeyboardInterrupt:
        printMinus('''Skipping:
\t%s
...
\t%s'''
    ...
    ADVICE = \
        'use :: smbclient //<server>/<share> -I <target ip> -N :: to mount shared drive anonymously'
    try:
...
def http(target, ports):
    ...
    SCRIPTS = \
        'http-methods,http-robots.txt,http-vuln-*,http-userdir-enum,http-iis-webdav-vuln,http-majordomo2-dir-traversal,http-axis2-dir-traversal,http-tplink-dir-traversal,http-useragent-tester'

    portString = ''
    for port in ports:
        port = port.split('/')[0]
        portString += '%s,' % port
    NMAPSCAN = 'nmap -T4 -p %s -sC -sV --script="%s" %s' % (portString,
            SCRIPTS, target)
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
    ...
    WEBDAVSCAN = \
        "nmap -T4 -p %s --script http-webdav-scan %s -d 2>/dev/null | grep 'http-webdav-scan %s'" \
        % (portString, target, target)
    try:
        webdavscanResults = subprocess.check_output(WEBDAVSCAN,
                shell=True)
    ...
    for port in ports:
        ...
        urlDir = ['/']
        port = port.split('/')[0]
        printStd('Trying to brute-force HTTP directories on port %s'
                 % port)
        DIRB = \
            'gobuster -x php -l -t 100 -u http://%s:%s -w /usr/share/wordlists/dirb/common.txt' \
            % (target, port)
        try:
            ...
            path = writeToFile(target, NAME, content)
            printPlus('Finished HTTP brute-force against port %s: %s - Found: %s'
                       % (port, path, len(urlArr)))
        except KeyboardInterrupt:
            printMinus('Skipping:\n\t%s' % DIRB)
        except Exception:
        ...
        for url in urlDir:
            ...
            try:
                printStd('Crawling %s for vulnerabilities (nikto)'
                         % url)

                niktoResults = subprocess.check_output(NIKTO,
                        shell=True)
                ...
                path = writeToFile(target, NAME, content)
                printPlus('Finished crawling %s for vulnerabilities (nikto): %s'
                           % (url, path))
            except KeyboardInterrupt:
            ...
            SCRIPTARGS = \
                'http-shellshock.uri=%(url)s,http-backup-finder.url=%(url)s,http-config-backup.path=%(url)s,http-default-accounts.category=web,http-default-accounts.basepath=%(url)s,httpspider.url=%(url)s,http-method-tamper.uri=%(url)s,http-passwd.root=%(url)s,http-phpmyadmin-dir-traversal.dir=%(url)s,http-phpself-xss.uri=%(url)s,http-rfi-spider.url=%(url)s,http-sitemap-generator.url=%(url)s,http-sql-injection.url=%(url)s,http-unsafe-output-escaping.url=%(url)s' \
                % {'url': urlparse.urlparse(url).path}
            VULNSCAN = \
                'nmap -T4 -p %s %s --script="%s" --script-args="%s" 2>&1' \
                % (port, target, SCRIPTS, SCRIPTARGS)
            try:
                printStd('Crawling %s for vulnerabilities (nmap)' % url)
                vulnscanResults = subprocess.check_output(VULNSCAN,
                        shell=True)
                ...
                path = writeToFile(target, NAME, content)
                printPlus('Finished crawling %s for vulnerabilities (nmap): %s'
                           % (url, path))
            except KeyboardInterrupt:
...
def https(target, ports):
    ...
    SCRIPTS = \
        'http-methods,http-robots.txt,http-vuln-*,http-shellshock,http-userdir-enum,http-iis-webdav-vuln,http-majordomo2-dir-traversal,http-axis2-dir-traversal,http-tplink-dir-traversal,http-useragent-tester,ssl-*'
    ...
    for port in ports:
        ...
        urlDir = ['/']
        port = port.split('/')[0]
        printStd('Trying to brute-force HTTPS directories on port %s'
                 % port)
        DIRB = \
            'gobuster -x php -l -t 100 -u https://%s:%s -w /usr/share/wordlists/dirb/common.txt' \
            % (target, port)
        try:
            dirbResults = subprocess.check_output(DIRB, shell=True)
            ...
            content = '%s' % printInBox(DIRB, dirbResults)
            path = writeToFile(target, NAME, content)
            printPlus('Finished HTTPS brute-force against port %s: %s - Found: %s'
                       % (port, path, len(urlArr)))
        except KeyboardInterrupt:
            printMinus('Skipping:\n\t%s' % DIRB)
        except Exception:
        ...
        for url in urlDir:
            ...
            SCRIPTARGS = \
                'http-backup-finder.url=%(url)s,http-config-backup.path=%(url)s,http-default-accounts.category=web,http-default-accounts.basepath=%(url)s,httpspider.url=%(url)s,http-method-tamper.uri=%(url)s,http-passwd.root=%(url)s,http-phpmyadmin-dir-traversal.dir=%(url)s,http-phpself-xss.uri=%(url)s,http-rfi-spider.url=%(url)s,http-sitemap-generator.url=%(url)s,http-sql-injection.url=%(url)s,http-unsafe-output-escaping.url=%(url)s' \
                % {'url': urlparse.urlparse(url).path}
            VULNSCAN = \
                'nmap -T4 -p %s %s --script="%s" --script-args="%s" 2>&1' \
                % (port, target, SCRIPTS, SCRIPTARGS)
...
def snmp(target, ports):
    ...
    for port in ports:
        port = port.split('/')[0]
        portString += '%s,' % port
    NMAPSCAN = 'nmap -sU -p %s --script="snmp-*" %s' % (portString,
            target)

    ONESIXTYONE = \
        'onesixtyone -c /usr/share/doc/onesixtyone/dict.txt %s 2>&1' \
        % target
    ADVICE = \
        'If match community string, use :: snmpwalk -c <community string> -v1 %s :: to enumerate' \
        % target
    foundCount = 0
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
        ...
        onesixtyoneResults = subprocess.check_output(ONESIXTYONE,
                shell=True)
    ...
    if foundCount > 1:
        ...
        WALK = \
            "for s in $(onesixtyone -c /usr/share/doc/onesixtyone/dict.txt %s | grep %s | cut -d ' ' -f 2 | sed -e 's/\[//g' -e 's/\]//g');do snmpwalk -c $s -v1 %s;done" \
...
def ms_sql(target, ports):
    ...
    BRUTE = \
        'medusa -h %s -U /usr/share/wordlists/metasploit/default_users_for_services_unhash.txt -P /usr/share/wordlists/metasploit/default_pass_for_services_unhash.txt -M mssql -L -f 2>&1' \
        % target
    try:
        bruteResults = subprocess.check_output(BRUTE, shell=True)
...
def mysql(target, ports):
    ...
    BRUTE = \
        'medusa -h %s -U /usr/share/wordlists/metasploit/default_users_for_services_unhash.txt -P /usr/share/wordlists/metasploit/default_pass_for_services_unhash.txt -M mysql -L -f 2>&1' \
        % target
    try:
        bruteResults = subprocess.check_output(BRUTE, shell=True)
...
def nfs(target, ports):
    ...
    ADVICE = \
        'Use :: mount -t ntf %s:[share] /mnt/%s -o nolock :: to mount share'
    try:
        showmountResults = subprocess.check_output(SHOWMOUNT,
                shell=True)
...
%s'''
    ...
    for port in ports:
        ...
        portString += '%s,' % port
    NMAPSCAN = \
        'nmap -T4 -p %s -sV -sC --script="nfs-showmount,nfs-ls" %s' \
        % (portString, target)
    try:
        nmapResults = subprocess.check_output(NMAPSCAN, shell=True)
